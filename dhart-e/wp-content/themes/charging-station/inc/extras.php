 ...
 * @return array
 */
function chargingstation_body_classes( $classes ) {
...
add_filter( 'body_class', 'chargingstation_body_classes' );
...
if (!function_exists('chargingstation_str_replace_assoc')) {

    /**
     * chargingstation_str_replace_assoc
     * @param  array $replace
     * @param  array $subject
     * @return array
     */
    function chargingstation_str_replace_assoc(array $replace, $subject) {
 ...
 */
function chargingstation_add_to_cart_fragment( $fragments ) {
	...
	<a class="nav-link cart-icon-wrap header-cart" href="javascript:void(0);" role="button" data-bs-toggle="collapse" data-bs-target="#Cart">
		...
		?>	
			<span><?php echo esc_html_e('0','charging-station'); ?></span>
		<?php
		}
		?>
	</a>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'chargingstation_add_to_cart_fragment' );
...
if ( ! function_exists( 'chargingstation_breadcrumb_title' ) ) {
	function chargingstation_breadcrumb_title() {
		...
		elseif ( is_day() ) : 
			...
			printf( __( 'Daily Archives: %s', 'charging-station' ), get_the_date() );
		
		elseif ( is_month() ) :
		
			printf( __( 'Monthly Archives: %s', 'charging-station' ), (get_the_date( 'F Y' ) ));
			
		elseif ( is_year() ) :
		
			printf( __( 'Yearly Archives: %s', 'charging-station' ), (get_the_date( 'Y' ) ) );
			
		elseif ( is_category() ) :
		
			printf( __( 'Category Archives: %s', 'charging-station' ), (single_cat_title( '', false ) ));

		elseif ( is_tag() ) :
		
			printf( __( 'Tag Archives: %s', 'charging-station' ), (single_tag_title( '', false ) ));
			
		elseif ( is_404() ) :

			printf( __( 'Error 404', 'charging-station' ));
			
		elseif ( is_author() ) :
		
			printf( __( 'Author: %s', 'charging-station' ), (get_the_author( '', false ) ));		
...
function chargingstation_breadcrumbs() {
	
	$showOnHome	= esc_html__('1','charging-station'); 	// 1 - Show breadcrumbs on the homepage, 0 - don't show
	$delimiter 	= '';   // Delimiter between breadcrumb
	$home 		= esc_html__('Home','charging-station'); 	// Text for the 'Home' link
	$showCurrent= esc_html__('1','charging-station'); // Current post/page title in breadcrumb in use 1, Use 0 for don't show
    ...
    if ( is_category() ) 
	{
		...
		echo $before . esc_html__('Archive by category','charging-station').' "' . esc_html(single_cat_title('', false)) . '"' .$after;
		
	} 
	
	elseif ( is_search() ) 
	{
		echo $before . esc_html__('Search results for','charging-station').' "' . esc_html(get_search_query()) . '"' . $after;
	} 
	...
	elseif ( is_tag() ) 
	{
		echo $before . esc_html__('Posts tagged ','charging-station').' "' . esc_html(single_tag_title('', false)) . '"' . $after;
	} 
	
	elseif ( is_author() ) {
		global $author;
		$userdata = get_userdata($author);
		echo $before . esc_html__('Articles posted by ','charging-station').'' . $userdata->display_name . $after;
	} 
	
	elseif ( is_404() ) {
		echo $before . esc_html__('Error 404 ','charging-station'). $after;
    }
	
    if ( get_query_var('paged') ) {
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
		echo ' ( ' . esc_html__('Page','charging-station') . '' . esc_html(get_query_var('paged')). ' )';
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
    }
...
add_action( 'wp_ajax_chargingstation_dismissed_notice_handler', 'chargingstation_ajax_notice_handler' );

/**
 * AJAX handler to store the state of dismissible notices.
 */
function chargingstation_ajax_notice_handler() {
    if ( isset( $_POST['type'] ) ) {
...
add_action( 'wp_ajax_install_act_plugin', 'chargingstation_admin_install_plugin' );

function chargingstation_admin_install_plugin() {
    /**
     * Install Plugin.
     */
